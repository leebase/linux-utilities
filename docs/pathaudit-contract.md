...
entries. It supports two exclusive invocation modes: the existing explicit-root
form, where every inspection root is named as a command-line operand, and an
additive `pathaudit --path` mode that reads the process `PATH` environment
variable once, splits that value into components, and classifies each component
with the same deterministic hazard rules used for explicit roots. Neither mode
searches for executables, walks directory contents, examines ancestor
directories, or remediates anything. One root or one PATH component represents
one inspection input; duplicate inputs remain distinct. This slice reports only
the hazards listed below. It does not inspect packages, running processes,
services, capabilities, ACLs, mount options, or ownership policy, and it
...
based on the root bytes and one `stat`-equivalent lookup of the final target.
...
execution. Existing explicit-root CLI behavior, finding taxonomy, output
escaping, ordering, resource limits, and exit-status meanings remain
compatibility requirements. Explicit-root mode still ignores the process
`PATH` environment variable; only the opt-in `--path` form reads it. Help
text, tests, README, the manual page, and changelog entries must describe
both exclusive forms; this contract is the source of truth.
...
process environment. It does not reread `PATH`, does not consult configuration
files or stdin, and does not perform executable search or directory traversal.
...
`NON_DIRECTORY_ROOT`, `GROUP_WRITABLE`, `WORLD_WRITABLE`. Every applicable
code is emitted once per input. Input order, locale, directory enumeration
...
The taxonomy is closed for this slice and is shared by the explicit-root form
and by every component produced by `pathaudit --path`. `EMPTY_ROOT` applies
when the operand or PATH component has zero bytes. An empty entry is not
silently translated to the current directory and receives no filesystem
lookup. An explicitly empty `PATH` therefore yields exactly one `EMPTY_ROOT`
finding and exit status `1` after successful classification; it is not an
operational error and must not be confused with unset `PATH` (`PATH_UNSET`,
status `2`). `RELATIVE_ROOT` applies to every nonempty input whose first byte
is not `/`, including `.` and `..`; the input is still looked up relative to
the process's initial working directory, so it may also receive a resolution
or permission finding.
...
directory, and it makes no claim that an unreported directory is safe. The
`--path` mode inherits these rules without adding executable-search, PATH
shadowing, or traversal hazards to the taxonomy.

## Acceptance Checks

Tests build a temporary directory tree without privilege changes and exercise
both exclusive modes. Explicit-root fixtures continue to pass every root on
the command line. Deterministic fixtures include: a private absolute
...
mixture with explicit roots as usage errors. Neither mode may search for
executables or traverse directories; fixtures that would only fail if traversal
or search occurred must remain clean.
...
pin the two-form synopsis for both exclusive modes.
...
Regression coverage pins the complete CLI (both modes), taxonomy, symlink,
permission, escaping, ordering, diagnostic, limit, and exit-status contract
above, including the `PATH_UNSET` versus empty-`PATH` distinction. Repository
README text, `man/pathaudit.1`, and changelog entries must describe both
modes and must not claim that pathaudit never reads `PATH`. Repository-wide
acceptance also runs all pre-existing `sysdiff`
shell, pytest, fixture, malformed-input, benchmark, sanitizer, and Valgrind
gates required by the existing quality surface. The new build and test wiring
must preserve every existing `sysdiff` command, output byte, diagnostic, exit
