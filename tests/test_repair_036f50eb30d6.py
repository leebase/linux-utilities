"""Regression test suite for repairing governed run failure 036f50eb30d6.

Sealed Evidence Directory:
    /home/lee/projects/linux-utilities-agent-orch-runs/036f50eb30d6

Lineage and Failure Analysis:
1. Origin Run Failure in 036f50eb30d6:
   Governed run 036f50eb30d6 executed the repair playbook for prior run 718b3902389f.
   At step_03_repair_and_verify_slice, the full test suite (python3 -m pytest) was run,
   collecting 897 items across all test modules.
   Three tests failed, causing the step and the entire run to fail closed:
     - FAILED tests/test_repair_337b9a6cea80.py::test_user_test_result_command_claims_confirmed_against_build_sysdiff
     - FAILED tests/test_repair_337b9a6cea80.py::test_user_journeys_execution_verified_validator_rule
     - FAILED tests/test_repair_4982e77d9cc9.py::test_current_user_test_result_conformance

2. Root Cause Analysis:
   - Invalid Key in Journey Objects:
     artifacts/user-test/result.json recorded journey objects using the non-schema key
     "journey": "<name>" instead of the required canonical Draft202012 schema property
     "name": "<name>".
   - String Claims in commands_run:
     In each journey of artifacts/user-test/result.json, commands_run contained plain string
     entries (e.g. "build/sysdiff --help") rather than structured dictionary objects with
     "command": str and "exit_code": int.
   - AttributeError on Claim Access:
     In tests/test_repair_337b9a6cea80.py, the test iterated over commands_run and performed
     claim.get("command", ""). Because claim was a string, Python raised:
         AttributeError: 'str' object has no attribute 'get'
   - Missing Top-Level findings Property:
     The canonical USER_JOURNEYS_RESULT_SCHEMA requires ["journeys", "findings"].
     artifacts/user-test/result.json omitted the required top-level "findings" property entirely.
   - Orchestrator Validator Rejection:
     ...
     because of schema invalidity, stating:
     "User-test result artifacts/user-test/result.json does not match the result schema,
      so its commands_run claims cannot be re-executed".
   - Schema Validation Failure in Prior Repair Suite:
     tests/test_repair_4982e77d9cc9.py (test_current_user_test_result_conformance) failed with
     42 ValidationError instances because of missing "name", missing "findings", and
     "build/sysdiff --help" not being of type "object".

3. Repair Acceptance Contract:
   - AC-1: Result Artifact Schema Conformance & Structure Integrity.
     artifacts/user-test/result.json conforms strictly to Draft202012 USER_JOURNEYS_RESULT_SCHEMA.
     Root contains required properties "journeys" and "findings".
     Every journey contains "name" (not "journey"), "status", "steps_taken", and "commands_run".
     Every claim in commands_run is an object with "command" (str) and "exit_code" (int).
   - AC-2: Command Claims Verification & Attribute/Type Safety.
     Claims in commands_run are dictionary objects, preventing AttributeError on .get().
     Every command claim begins with "build/sysdiff" without shell operators.
     Every allowlisted command claim re-executed from ROOT matches its claimed exit_code.
   - AC-3: Orchestrator Validation Rule Execution Fidelity.
     agent_orch validator user_journeys_execution_verified confirms result claims against
     tests/user_journeys_manifest.json with outcome.passed is True.
     Every journey in tests/user_journeys_manifest.json is represented in result.json with status "passed".
   - AC-4: User Journey Manifest Synchronization & Authority Preservation.
     tests/user_journeys_manifest.json and journeys/user_journeys_manifest.json are identical parsed
     objects conforming to USER_JOURNEYS_MANIFEST_SCHEMA across all 21 journeys.
     command_allowlist is strictly ["build/sysdiff"].
   - AC-5: Clean Execution Environment & Non-Product Blast Radius.
     sys.path contains no foreign project contamination (e.g. employee-contract).
     Workspace root remains clean of ad-hoc generator scripts.
...
SEALED_EVIDENCE_DIR = Path("/home/lee/projects/linux-utilities-agent-orch-runs/036f50eb30d6")
...
def _load_result_dict() -> dict[str, Any]:
    """Helper to load and return the parsed user test result JSON."""
    assert USER_TEST_RESULT.exists(), f"User test result artifact missing: {USER_TEST_RESULT}"
    content = USER_TEST_RESULT.read_text(encoding="utf-8")
...
def test_user_test_result_file_exists_and_is_valid_json() -> None:
    """AC-1: artifacts/user-test/result.json must exist and parse cleanly as valid JSON."""
    assert USER_TEST_RESULT.exists(), f"User test result artifact missing: {USER_TEST_RESULT}"
...
def test_user_test_result_root_keys_contain_journeys_and_findings() -> None:
    """AC-1: Root object must contain required properties 'journeys' and 'findings'."""
    data = _load_result_dict()
    ...
    assert "findings" in data, (
        f"Missing required property 'findings' in {USER_TEST_RESULT}. "
        "Run 036f50eb30d6 failed because the root object lacked the 'findings' array."
    )
    assert isinstance(data["findings"], list), f"'findings' must be a list, got {type(data['findings']).__name__}"


def test_user_test_result_journeys_use_name_key_not_journey_key() -> None:
    """AC-1: Every journey must use 'name' as its key, NOT 'journey'.

    Regression for run 036f50eb30d6: the evaluator emitted:
        {"journey": "A user ...", "status": "passed", ...}
    instead of the canonical Draft202012 schema property:
        {"name": "A user ...", "status": "passed", ...}
    causing 21 ValidationError failures.
    """
    data = _load_result_dict()
    ...
    for idx, j in enumerate(journeys):
        ...
        assert "name" in j, (
            f"Journey [{idx}] missing required property 'name'. "
            f"Found keys: {list(j.keys())}. "
            "Using 'journey' instead of 'name' was the primary failure in run 036f50eb30d6."
        )
        assert isinstance(j["name"], str) and len(j["name"].strip()) > 0, (
            f"Journey [{idx}] 'name' must be a non-empty string"
        )
        assert "journey" not in j, (
            f"Journey [{idx}] contains forbidden deprecated key 'journey'. Must use 'name'."
        )


def test_user_test_result_commands_run_are_objects_not_bare_strings() -> None:
    """AC-1: Every claim in commands_run must be a dict object with 'command' and 'exit_code'.

    Regression for run 036f50eb30d6: commands_run contained bare strings:
        "commands_run": ["build/sysdiff --help"]
    which caused AttributeError: 'str' object has no attribute 'get' in test_repair_337b9a6cea80.py
    and schema validation failure in test_repair_4982e77d9cc9.py.
    """
    data = _load_result_dict()
    journeys = data.get("journeys", [])
    ...
    for idx, j in enumerate(journeys):
        ...
        assert isinstance(commands_run, list), (
            f"[{j_name}] commands_run must be a list, got {type(commands_run).__name__}"
        )
        ...
        for c_idx, claim in enumerate(commands_run):
            assert isinstance(claim, dict), (
                ...
                "In run 036f50eb30d6, claims were strings rather than objects, breaking claim.get('command')."
            )
...
def test_user_test_result_findings_structure_integrity() -> None:
    """AC-1: If findings are present in result.json, each must satisfy canonical schema properties."""
    data = _load_result_dict()
...
def test_user_test_result_satisfies_canonical_schema() -> None:
    """AC-1: artifacts/user-test/result.json strictly satisfies USER_JOURNEYS_RESULT_SCHEMA."""
...
def test_user_test_result_command_claims_safe_dict_access_no_attribute_error() -> None:
    ...
    Direct regression test for run 036f50eb30d6:
...
def test_user_test_result_command_claims_confirmed_against_build_sysdiff() -> None:
    ...
    Ensures that tester claims are authentic, re-executable, and verified.
    """
    data = _load_result_dict()
...
def test_user_journeys_execution_verified_orchestrator_rule() -> None:
    """AC-3: agent_orch validator user_journeys_execution_verified rule passes on result.json.

    Directly invokes the system validator that failed in run 036f50eb30d6.
    """
...
def test_all_manifest_journeys_represented_in_result_with_status_passed() -> None:
    """AC-3: Every journey in tests/user_journeys_manifest.json is reported as 'passed' in result.json."""
    manifest = _load_manifest_dict()
    result = _load_result_dict()
...
def test_user_journeys_manifest_exists_and_conforms_to_schema() -> None:
    """AC-4: tests/user_journeys_manifest.json exists and conforms to USER_JOURNEYS_MANIFEST_SCHEMA."""
    manifest = _load_manifest_dict()
...
def test_user_journeys_manifest_contains_all_21_journeys() -> None:
    """AC-4: Manifest contains exactly the 21 enumerated user journeys without omission or drift."""
    manifest = _load_manifest_dict()
...
def test_user_journeys_manifests_are_synchronized() -> None:
    """AC-4: tests/user_journeys_manifest.json and journeys/user_journeys_manifest.json are identical."""
...
def test_no_foreign_cross_project_sys_path_contamination() -> None:
    """AC-5: Test modules must not hardcode foreign repository paths like employee-contract into sys.path."""
...
def test_workspace_root_clean_of_adhoc_scripts() -> None:
    """AC-5: Workspace root must remain clean of untracked generator or ad-hoc scripts."""
    forbidden_root_scripts = [
        "generate_result.py",
        "make_result.py",
        "repair.py",
        "test.py",
    ...
    for script_name in forbidden_root_scripts:
...
def test_sysdiff_c17_source_and_makefile_hash_pins() -> None:
    """AC-5: src/sysdiff.c and Makefile match canonical baseline SHA-256 hashes."""
...
def test_sysdiff_compare_three_state_exit_contract(tmp_path: Path) -> None:
    """AC-5: sysdiff compare preserves the 3-state exit status contract (0, 1, 2)."""
