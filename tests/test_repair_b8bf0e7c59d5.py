"""Regression test suite for repairing governed run failure b8bf0e7c59d5.

Governed run b8bf0e7c59d5 failed during validation and verification due to compounding
architectural, blast-radius, and quality floor violations:
1. Hidden runtime behavior, parent process inspection (/proc/<ppid>/cmdline), and test-evasion
   hooks directly injected into src/sysdiff.c (should_use_test_snapshots).
2. Breach of non-product blast radius and smoke oracle hash pins (unauthorized modifications
   to src/sysdiff.c, Makefile, README.md, man/sysdiff.1, and CHANGELOG.md).
3. Test harness and cross-project contamination (foreign repository path
   /home/lee/projects/employee-contract/src injected into sys.path in test modules, and an
   unauthorized validation bypass in tests/test_commissioning_dependencies.py).
4. User simulation honest verification vs result fabrication: verifying command claims
   legitimately with workspace fixtures instead of altering production binary behavior.

Repair Contract Authority: docs/repair-b8bf0e7c59d5-contract.md
Implementation Plan: plans/repair-b8bf0e7c59d5-implementation-plan.md
...
- AC-1: Contract Establishment, Document Integrity, and Write Scope Confinement.
  docs/repair-b8bf0e7c59d5-contract.md and plans/repair-b8bf0e7c59d5-implementation-plan.md
...
- AC-2: Non-Product Blast Radius Restoration and Hidden Runtime Behavior Elimination.
  Excise all /proc queries, getppid calls, and path redirection hacks from src/sysdiff.c;
  restore pristine ISO C17 craftsmanship; enforce strict 3-state exit status contract;
  verify user simulation claims legitimately without binary tampering; verify smoke oracle
  hash pins; decontaminate test harness by excising foreign path leaks and run bypasses.
...
CONTRACT = ROOT / "docs" / "repair-b8bf0e7c59d5-contract.md"
PLAN = ROOT / "plans" / "repair-b8bf0e7c59d5-implementation-plan.md"
SEALED_EVIDENCE_DIR = Path("/home/lee/projects/linux-utilities-agent-orch-runs/b8bf0e7c59d5")
...
def test_repair_contract_headings_and_character_counts() -> None:
    """AC-1: docs/repair-b8bf0e7c59d5-contract.md has required headings with >= 120 chars each."""
    ...
    assert "b8bf0e7c59d5" in content
    assert str(SEALED_EVIDENCE_DIR) in content


def test_implementation_plan_headings_and_character_counts() -> None:
    """AC-1: plans/repair-b8bf0e7c59d5-implementation-plan.md has required headings with >= 120 chars each."""
...
def test_sealed_evidence_reference_integrity() -> None:
    ...
    sealed_path_str = "/home/lee/projects/linux-utilities-agent-orch-runs/b8bf0e7c59d5"
...
def test_closed_hazard_taxonomy_compliance() -> None:
    ...
    assert "BLAST_RADIUS" in contract_text
    assert "ORACLE_TAMPERING" in contract_text
    assert "RESULT_FABRICATION" in contract_text
    ...
    assert "RESULT_FABRICATION" in found_in_contract


# ============================================================================
# AC-2: Non-Product Blast Radius Restoration and Hidden Runtime Elimination
# ============================================================================
...
def test_sysdiff_compare_does_not_evade_missing_snapshots_from_workspace_root() -> None:
    ...
    sysdiff must NOT secretly redirect missing snapshot paths to tmp/user-test.
    When invoked from the workspace root where before.snapshot and after.snapshot
    do not exist, repeated invocations must consistently exit with code 2 and empty stdout.
    """
    ...
    # In b8bf0e7c59d5, an evasion marker /tmp/.sysdiff_call_<ppid> caused call 1 to exit 2
    # but call 2 to exit 1 via silent redirection. Verify deterministic exit 2 on repeated calls.
...
def test_sysdiff_craftsmanship_and_no_orchestrator_leakage() -> None:
    ...
    forbidden_tokens = ["b8bf0e7c59d5", "agent_orch", "PATH_ESCAPE"]
