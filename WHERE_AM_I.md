...
The current orientation is the infrastructure recovery record in
`docs/mission-infrastructure-recovery.md`. The real checkout and autonomous
linked worktree have matching expected identity, but immutable metadata
protection is unavailable on this host because `chattr +i` is denied. The
live scratch root is now correctly pruned by target and retention policy;
autonomous scheduling remains paused. Routing authority is unresolved and
the latest governed smoke run failed, so this mission is not ready for re-arm.

The recovery sprint was executed through AgentFlow run `fe54e33800e9` after
strict playbook lint passed. Its docs-only worker produced the execution
report, and the evidence chain verified cleanly, but the route-separated
`pi_cli` semantic gate failed on both attempts because no `openai-codex` API
key was available. The real checkout closeout passed the full suite (337
passed, 18 skipped), identity checks, protected-path dry-run, pruner dry-run,
shell syntax, and strict lint. Trusted-boundary enforcement, deterministic
routing, provider-backed semantic judging, and successful current smoke still
block re-arm.
