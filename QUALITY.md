...
12. `make test-valgrind` — GCC debug rebuild under Valgrind memcheck with
    `--error-exitcode=99`, wrapping both the shell suite and the full pytest
    suite
...
- Malformed-input fuzz coverage is the deterministic corpus in
  ...
  open-ended fuzzing. Performance benchmarking is gated via `benchmark-check`
  inside `make quality` and via standalone `make benchmark`.
